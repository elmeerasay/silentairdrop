...
const logHeader = async () => {
    ...
    console.log(`4. Tern     : ${balances.tern} TERN\n`);
};
...
const getTernProvider = () => {
    const rpcUrl = 'https://brn.rpc.caldera.xyz/http';
    const chainId = 6636130;
    return new ethers.providers.JsonRpcProvider(rpcUrl, { chainId });
};
...
const sendTransaction = async () => {
    try {
        ...
        const balance = 'Error';
        const ternBalance = 'Error';
        const blockNumber = 'Error';
        const txHash = 'Error';

        console.error(`Nomor\t: ${transactionCount}\nBalance\t: ${balance}\nBalance\t: ${ternBalance}\nBlock\t: ${blockNumber}\nTx hash\t: ${txHash}`); //', error);
    }
    transactionCount++; // Increment the transaction count
};
...
const getAllBalances = async () => {
    ...
    const balances = await Promise.all([
        ...
        providers.tern.getBalance(wallet.address)
    ]);
    ...
    return {
        ...
        tern: ethers.utils.formatEther(balances[3])
    };
};
...
const main = async () => {
    ...
    while (true) {
        ...
        if (['1', '2', '3', '4'].includes(currentChoice)) {
            ...
            currentChoice = null; // Reset pilihan jika tidak valid
        }
    }
};
